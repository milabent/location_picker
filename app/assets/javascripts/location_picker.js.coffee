...
class LocationSelector
  ...
  handleActions: () =>
    infoPanel = $('.alert.alert-info', @context)
    errorPanel = $('.alert.alert-danger', @context)
    $('.reset', @context).click () => @resetPosition()
    $('.reload', @context).click () =>
  ...
  loadMap: () =>
    mapId = addUniqIdToMapDiv(@context.find('.map'))
    @map = new L.Map mapId,
      zoomControl: false
